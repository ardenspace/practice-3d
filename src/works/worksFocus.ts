...
export interface WorksFocusOptions {
   ...
   *
   * 방울에 초점이 있었던 경우도 같은 통로로 들어온다. 씬은 방울 N개짜리
   * 그림이지만 DOM에서는 초점 정거장 하나이므로(Requirement 11) 그 정거장이
   * 지금 서 있는 방울의 slug를 같은 표식으로 달고, 여기 그대로 실린다
   * (scene/Home.tsx).
   */
  slug?: string | null
  /** 초점을 옮길 문서. 기본은 지금 문서. */
 ...
 * 빼앗지 않는다. 셸 안에 있었으면 그 초점이 어느 작품을 가리키고 있었는지
 * (`WORK_ITEM_ATTR`를 단 조상)를 함께 실어 보낸다. 표식을 단 조상은 목록
 * 항목일 수도, 지금 그 방울에 서 있는 씬의 초점 정거장일 수도 있다 — 두
 * 자리를 여기서 구별하지 않는 것이 곧 "방울에 있던 초점도 그 작품의 항목으로
 * 간다"이다. 어느 작품도 가리키지 않는 자리에 있었다면 slug는 null이고,
 * 초점은 목록의 기본 자리로 간다.
 *
