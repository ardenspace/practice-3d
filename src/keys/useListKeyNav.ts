...
// 하고 — 이 조합이 우리 것인지, 키의 뜻, 가로챌지 여부, 다음 자리 — 여기는
// 그 판정을 창(window)의 keydown 하나에 이어 붙인다 (그 이어 붙이는 일은 세
// 배선이 나눠 쓰는 useLayerKeyDown이 맡는다). 두 배선의 차이는 항목이 무엇이냐 하나다:
...
export function useListKeyNav(options: ListKeyNavOptions): void {
  useLayerKeyDown((event) => {
    const { surfaceRef, itemSelector, onExit, trapTab } = options
