...
// ── 씬을 보조기술에 드러내는 층의 문구 (B6) ──
// 3D 캔버스는 그 자체로 보조기술에 아무것도 드러내지 않는다. 아래 문구들이
// 화면을 보지 않는 방문자가 씬에서 실제로 듣게 되는 전부다.

// 씬의 초점 정거장 이름 (Requirement 11의 그 한 자리). 탭으로 들어온
// 방문자가 "여기가 무엇인가"를 이 이름으로 듣는다.
export const SCENE_LABEL = '작품 방울 씬'

// 씬의 작품 방울들을 그대로 옮겨 놓은 목록의 이름 (B6 ①). 화면에서는 방울이
// 우주에 흩어져 있지만, 소리로는 등록부 순서의 목록 하나로 들린다.
export const SCENE_BUBBLES_LABEL = '떠 있는 작품 방울'

// 키보드 조작 방법 (Requirement 43). 두 갈래로 알려지는 같은 한 문장이다 —
// 소리로 듣는 사람에게는 씬 정거장의 설명으로 항상 읽히고, 화면에는 방문자가
// 키보드를 쓰기 시작한 순간 나타난다. 문구가 한 곳에 있으므로 두 갈래가
// 서로 다른 말을 하지 않는다.
export const KEYBOARD_HINT =
  '방향키로 방울 사이를 옮기고, 엔터로 그 작품을 엽니다. Esc를 누르면 돌아옵니다.'

/**
 * 씬 안에서 지금 서 있는 자리 (B6 ①: 초점이 있는 항목의 이름과 전체 안에서의
 * 위치). 화면에는 초점 고리로 보이는 것이 소리로는 이 한 줄이 된다.
 * 시그니처가 문자열과 숫자만 받는 이유는 토큰 모듈이 등록부나 씬을
 * 임포트하지 않기 위해서다.
 */
export function sceneBubbleStatus(
  title: string,
  index: number,
  total: number,
): string {
  return `${title}, ${total}개 중 ${index + 1}번째`
}

// 목록이 열렸다는 사실 (Requirement 21). 열리면 아이콘이 사라지므로
// aria-expanded로 알릴 물건이 남지 않는다 — 이 문구가 그 자리를 맡는다.
export const WORKS_OPENED_STATUS = '작품 목록이 열렸습니다.'
