...
//   나오기 때문이다. 씬이 없는 갈래에서는 이 층이 통째로 없고, 대신 눈에
//   보이는 폴백 안내 띠 하나가 그 갈래의 알림 영역을 겸한다 (B6 ⑥) —
//   두 갈래는 함께 그려지지 않으므로 화면에 알림 영역은 언제나 하나다.
//
...
//
// 이 띠는 눈에 보이는 안내이자 소리로 들리는 안내다 (B6 ⑥) — 아래 렌더에서
// 이 요소 자체가 알림 영역이 된다. 숨은 사본을 따로 두지 않는 이유는 그러면
// 같은 말이 화면과 소리로 갈라져 두 벌이 되고, 둘 중 하나만 고쳐지는 날이
// 오기 때문이다.
...
export default function Home() {
  ...
  if (!fallback.showScene) {
    ...
    // 않는다).
    //
    // 안내 문구는 화면에만 있어서는 안 된다 (B6 ⑥). 방문자는 이 화면을
    // 요청한 적이 없으므로 화면 어딘가에 사과가 적혀 있다는 사실 자체를 알
    // 이유가 없다 — 스스로 찾아 읽을 문구가 아니라 들려야 하는 소식이다.
    // 그래서 눈에 보이는 그 띠가 곧 알림 영역이다: 같은 요소 하나가 두
    // 감각을 다 맡으므로 화면과 소리가 서로 다른 말을 할 수 없다.
    //
    // polite가 아니라 alert인 이유는 두 가지다. 하나, 이 소식은 방문자가
    // 화면을 만지기 전에 닿아야 한다 — 씬을 기대하고 온 사람이 영문도 모른
    // 채 목록을 훑기 시작하면 설명이 뒤늦게 와도 소용이 없다. 둘, 이 문구는
    // 언제나 방금 붙은 요소로 나타난다(이 셸이 뜨는 순간, 또는 씬이 무너져
    // 화면이 갈리는 순간). 이미 있던 영역의 글자가 바뀌는 것이 아니라 영역째
    // 새로 들어오는 자리에서는 assertive 쪽이 실제로 읽힐 확률이 높다.
    // 씬 쪽 알림(polite)과는 한 화면에 함께 있을 수 없다 — 그쪽은 씬 셸
    // 안에 살고, 이 갈래는 씬 셸을 아예 그리지 않는다. 알림 영역은 어느
    // 화면에서든 하나다.
    return (
      <>
        {showNotice && (
